...
class NormalizationSummaryCheckActivityTest {
  ...
  void testShouldRunNormalizationRecordsCommittedOnFirstAttemptButNotCurrentAttempt() throws IOException {
    ...
    // Attempt 3 did not commit records, normalization did not run
    ...
    final AttemptNormalizationStatus attempt3 = new AttemptNormalizationStatus(3, Optional.of(0L), true);
    Mockito.when(mJobPersistence.getAttemptNormalizationStatusesForJob(JOB_ID)).thenReturn(List.of(attempt1, attempt2, attempt3));

    Assertions.assertThat(true).isEqualTo(normalizationSummaryCheckActivity.shouldRunNormalization(JOB_ID, 3L));
  }

  @Test
  void testShouldRunNormalizationRecordsCommittedOnCurrentAttempt() throws IOException {
    // Attempt 1 and 2 committed records and normalization succeeded
    // Attempt 3 committed records and normalization has not run yet
    ...
    final AttemptNormalizationStatus attempt3 = new AttemptNormalizationStatus(3, Optional.of(30L), true);
    Mockito.when(mJobPersistence.getAttemptNormalizationStatusesForJob(JOB_ID)).thenReturn(List.of(attempt1, attempt2, attempt3));
    Assertions.assertThat(true).isEqualTo(normalizationSummaryCheckActivity.shouldRunNormalization(JOB_ID, 3L));
  }
  ...
  void testShouldRunNormalizationNoRecordsCommittedOnCurrentAttemptOrPreviousAttempts() throws IOException {
    ...
    final AttemptNormalizationStatus attempt3 = new AttemptNormalizationStatus(3, Optional.of(0L), true);
    Mockito.when(mJobPersistence.getAttemptNormalizationStatusesForJob(JOB_ID)).thenReturn(List.of(attempt1, attempt2, attempt3));
    Assertions.assertThat(false).isEqualTo(normalizationSummaryCheckActivity.shouldRunNormalization(JOB_ID, 3L));
  }
  ...
  void testShouldRunNormalizationNoRecordsCommittedOnCurrentAttemptPreviousAttemptsSucceeded() throws IOException {
    ...
    final AttemptNormalizationStatus attempt1 = new AttemptNormalizationStatus(1, Optional.of(10L), false);
    final AttemptNormalizationStatus attempt2 = new AttemptNormalizationStatus(2, Optional.of(20L), false);
    final AttemptNormalizationStatus attempt3 = new AttemptNormalizationStatus(3, Optional.of(0L), false);
    Mockito.when(mJobPersistence.getAttemptNormalizationStatusesForJob(JOB_ID)).thenReturn(List.of(attempt1, attempt2, attempt3));
    Assertions.assertThat(false).isEqualTo(normalizationSummaryCheckActivity.shouldRunNormalization(JOB_ID, 3L));
  }
